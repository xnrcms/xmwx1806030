$(function() {
		...
		uploader.on( 'uploadSuccess', function( file,data) {
			...
			if(data.code == 1 && jsonData.total > 0){
				...
				var sLi 	= "";
					...
					sLi += '<input type="hidden" name="file_tmp[]" value="' + data.url + '">';
					sLi += '<input type="hidden" name="file_ids[]" value="' + data.id + '">';
					sLi += '<input type="hidden" name="file_type[]" value="' + ext + '">';
					sLi += '<input type="hidden" name="file_name[]" value="' + data.url + '">';
			...
			if ( file.getStatus() === 'invalid' ) {
				...
				uploader.makeThumb( file, function( error, src ) {
					...
					if( isSupportBase64 ) {
						...
						$.ajax("/manage/uploadFile/preview", {
							method: 'POST',
							data: src,
							dataType:'json'
	...
	window.Manager = {
		...
		createFile : function(data){
			...
			if(data && data.list && data.list.length > 0){
				...
				for(; i < data.list.length; i++){
					...
					}).data('file', {name : t.name, url : t.url, mtime : t.create_time, list : data.list,ids : t.id});
		...
		delFile : function($file){
			...
			var data = {url : file.url, name : file.name, mtime : file.mtime,ids:file.ids};
			...
			$.ajax({
				...
				success : function(data){
					layer.msg(data.msg,{icon: (data.code == 1 ? 6 : 2),time:2000},function(){
						if(data.code == 1){
							$file.fadeOut(function(){ $(this).remove();});
						}
					});
				},
		...
		sure : function($file){
			...
			var sLi 	= "";
				...
				sLi += '<img src="' + data.url + '" width="100" height="100" onerror="this.src='+nopic+'">';
				sLi += '<input type="hidden" name="file_tmp[]" value="' + data.url + '">';
				sLi += '<input type="hidden" name="file_ids[]" value="' + data.ids + '">';
				sLi += '<input type="hidden" name="file_type[]" value="' + ext + '">';
				sLi += '<input type="hidden" name="file_name[]" value="' + data.url + '">';
				sLi += '<a href="javascript:void(0);">删除</a>';
				sLi += '</li>';
		...
		del:function(obj,ids){
			if(!confirm('确定删除吗?')) return false;
		    $.get(this.opts.delFileUrl,{ 'ids':ids},function(data){
					if(data.code == 1){
