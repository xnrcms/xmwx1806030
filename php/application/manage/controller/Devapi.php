...
class Devapi extends Base
{
    ...
    public function __construct()
    {
        ...
        //项目ID
        $this->project_id   = intval(session('apidoc_user_auth.project_id'));
        
        //项目info
        $this->project_info = session('apidoc_user_auth.project_info');
    }
	...
	public function index()
    {
        ...
        //APIDOC授权登录信息没有需要重新授权登录
        $this->apidoc_go_login();
        ...
        $project_name           = session('apidoc_user_auth.project_name');
        $project_name           = !empty($project_name)?'【<font color="red">'.$project_name.'</font>】项目':'';
	...
	public function add()
    {
        ...
        $info['user_id']                = session('apidoc_user_auth.apidoc_uid');
    ...
    public function debug($id)
    {
        ...
        $envs               = json_decode($this->project_info['envs'], true);
        $info               = $this->getDetail($id);
    ...
    private function apidoc_start_login()
    {
        $email          = input('post.email','');
        $password       = input('post.password','');

        if (empty($email)) $this->error('登录邮箱不能为空');
        if (empty($password)) $this->error('登录密码不能为空');

        //开始登录
        $parame['apikey']       = md5(config('extend.uc_auth_key'));
        $parame['email']        = $email;
        $parame['password']     = $password;
        $parame['password']     = $password;
        $parame['project_id']   = config('extend.apidoc_project_id');
        $parame['project_url']  = config('extend.apidoc_project_url');
        $parame['dev_ip']       = request()->ip();

        $res                    = apidocReq($parame,'login_api');

        if (!empty($res) && isset($res['code']) && $res['code'] == 200 ) {

            if (isset($res['userInfo']) && !empty($res['userInfo'])) {

                session('apidoc_user_auth', $res['userInfo']);
                session('apidoc_user_auth_sign', data_auth_sign($res['userInfo']));

                $this->success('授权登录成功',Cookie('__forward__')) ;
            }
        }else{

            if(!empty($res) && isset($res['msg']) && !empty($res['msg'])){
    ...
    //登录跳转
    private function apidoc_go_login()
    {
        $project_id         = session('apidoc_user_auth.project_id');

        if ($this->is_apidoc_login() > 0 && $project_id == config('extend.apidoc_project_id')) return true;

        if (request()->isAjax()) $this->error('APIDOC登录失效，请重新登录!',url('Devapi/apidoc_login'));
        
        header("Location:" . url('Devapi/apidoc_login'));exit;
    }

    private function is_apidoc_login()
    {
        $user = session('apidoc_user_auth');
        ...
        if (empty($user)) {
            ...
            return session('apidoc_user_auth_sign') == data_auth_sign($user) ? intval($user['apidoc_uid']) : 0;
        }
    }
    ...
    public function addModule()
    {
        ...
        //数据详情
        $info                           = $this->getDetail(0);
        $info['project_id']             = session('apidoc_user_auth.project_id');
        $info['user_id']                = session('apidoc_user_auth.apidoc_uid');
