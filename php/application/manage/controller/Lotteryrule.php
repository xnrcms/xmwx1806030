...
class Lotteryrule extends Base
{
    ...
    public function setodds($id)
    {
        ...
        if (request()->isPost())
        {
            ...
            if ($agent_odds > $odds || $agent_odds < $odds-$difference_value)
            $this->error("赔率设置不在平台规定范围内【".$odds."-".($odds-$difference_value)."】");

            //正确数据事例
            request()->post(['fieldName'=>'agent_odds']);
            request()->post(['dataId'=>$info['id']]);
            request()->post(['value'=>$parame['agent_odds']]);
