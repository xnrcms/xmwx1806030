...
class Article extends Base
{
	...
	public function index()
    {
        ...
        //页面头信息设置
        $pageData['isback']             = 0;
        $pageData['title1']             = '文章列表';
        $pageData['title2']             = '文章索引与管理';
    ...
    private function update()
    {
        ...
        $nickname                   = !empty($this->userInfo['nickname']) ? $this->userInfo['nickname'] : $this->userInfo['username'];
        //用户信息
    ...
    {
        $defaultData['getArticleCat']   = $this->getArticleCat();

        return $defaultData;
    }

    private function getArticleCat()
    {
        //获取列表数据
        $parame['uid']      = $this->uid;
        $parame['hashid']   = $this->hashid;
        ...
        //请求数据
        if (!isset($this->apiUrl['articlecat']) || empty($this->apiUrl['articlecat']))
        $this->error('未设置接口地址');

        $res                = $this->apiData($parame,$this->apiUrl['articlecat']);
        $data               = $this->getApiData() ;
        $listData           = (isset($data['lists']) && !empty($data['lists'])) ? $data['lists'] : [];
        ...
        if (!empty($listData)) {
            foreach ($listData as $key => $value) {
                $list[$value['id']] = $value['title_show'];
            }
        }
