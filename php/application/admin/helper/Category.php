...
class Category extends Base
{
    ...
    private function listData($parame)
    {
		...
		//数据分页步长定义
		$modelParame['limit']		= isset($parame['limit']) ? intval($parame['limit']) : 1000;
    ...
    private function saveData($parame)
    {
        ...
        $saveData['pid']            = isset($parame['pid']) ? intval($parame['pid']) : 0;
        $saveData['sort']           = isset($parame['sort']) ? intval($parame['sort']) : 1;
        $saveData['ctype']          = (isset($parame['ctype'])) ? intval($parame['ctype']) : 0;
        $saveData['icon']           = isset($parame['icon']) ? intval($parame['icon']) : 0;
        $saveData['description']    = isset($parame['description']) ? trim($parame['description']) : '';
        $saveData['status']         = isset($parame['status']) ? intval($parame['status']) : 0;
