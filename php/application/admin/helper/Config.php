...
class Config extends Base
{
    ...
    private function saveData($parame)
    {
        ...
        $saveData                   = [];
        $saveData['config_type']    = isset($parame['config_type']) ? $parame['config_type'] : '';
        $saveData['config']         = isset($parame['config']) ? $parame['config'] : '';
        //$saveData['parame']         = isset($parame['parame']) ? $parame['parame'] : '';

        //数据校验
        if(empty($saveData['config_type'])) return ['Code' => '100008', 'Msg'=>lang('100008',['config_type'])];
        ...
        if(empty($config)) return ['Code' => '100008', 'Msg'=>lang('100008',['config'])];
        //...
		
        $addConfig              = $dbModel->addConfigData($config,$saveData['config_type']);
    ...
    private function detailData($parame)
    {
        ...
        //数据ID
        $config_type        = isset($parame['config_type']) ? trim($parame['config_type']) : '';
        if (empty($config_type)) return ['Code' => '120023', 'Msg'=>lang('120023')];

        //数据详情
    	$info 				= $dbModel->getConfigData($config_type);

        //数据处理
        $info['mark_img_path']  = isset($info['mark_img']) ? get_cover($info['mark_img'],'path') : '';
