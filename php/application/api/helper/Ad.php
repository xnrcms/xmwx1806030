...
class Ad extends Base
{
    ...
    private function listData($parame)
    {
    	...
    	if (!empty($data)) {
    		...
    		foreach($data as $k=>$v){
                ...
                if (isset($v['imgid']))   $data[$k]['imgid_path']   = get_cover($v['imgid'],'url');
    		}
    	}
    ...
    private function saveData($parame)
    {
        ...
        $saveData['type']           = (isset($parame['type']) && !empty($parame['type'])) ? $parame['type'] : 1;
        $saveData['urls']           = isset($parame['urls']) ? $parame['urls'] : '';
        $saveData['pos_id']         = isset($parame['pos_id']) ? intval($parame['pos_id']) : 0;
        $saveData['imgid']          = isset($parame['imgid']) ? intval($parame['imgid']) : 0;
        ...
        $pos_name                   = model('ad_position')->where('id','eq',$saveData['pos_id'])->value('title');

        $saveData['pos_name']       = !empty($pos_name) ?  $pos_name : '';

        //检测广告位名称是否存在
        if ($dbModel->checkValue($saveData['title'],$id,'title')) {

            return ['Code' => '160002', 'Msg'=>lang('160002')];
        }

        //规避遗漏定义入库数据
        if (empty($saveData)) {
            
            return ['Code' => '120021', 'Msg'=>lang('120021')];
        }
