...
class PushController extends AdminController {
	...
	public function edit($id = 0){
		...
		//页面数据
		$info 			= M('push')->field(true)->find($id);
		if(false === $info){
			$this->error('获取配置信息错误');
		}
		$this->assign('info', $info);
	...
	protected function CustomerForm($index=0){
		...
		$FormData[0] = array(
		array('fieldName'=>'推送名称','fieldValue'=>'name','fieldType'=>'text','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入推送名称"'),
