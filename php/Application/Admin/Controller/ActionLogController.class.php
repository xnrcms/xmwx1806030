...
class ActionLogController extends AdminController {
    ...
    public function index() {
        ...
        if (!empty($list)) {
            foreach ($list as $k => $v) {
                ...
                //登录用户名
                $list[$k]['user_name'] 		= M('ucenter_member')->where(array('id'=>$v['user_id']))->getField('username');
                //登录ip
                $list[$k]['action_ip'] 		= long2ip($v['action_ip']);
            }
        }
        ...
        $this->NavTitle = '登录管理';
       	$this->assign('SmallNav', array('登录管理', '登录列表'));
        //记录当前列表页的cookie
    ...
    public function del() {
    	...
    	if (M('actionLog')->where($map)->delete()) {
    		//记录行为
    		action_log('actionLog', $ids, UID);
    		//数据返回
