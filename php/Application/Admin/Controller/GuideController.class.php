...
class GuideController extends AdminController {
	...
	public function index(){
		 ...
		 * */
		$RelationTab				= array(
			'picture'=>array('Ralias'=>'pic','Ron'=>'pic ON pic.id=main.picture','Rfield'=>array('path')),
		);
	...
	public function edit($id = 0){
		...
		//页面数据
		$info 			= M('Guide')->field(true)->find($id);
		if(false === $info){
			$this->error('获取配置信息错误');
		}
		$this->assign('info', $info);
	...
	protected function CustomerForm($index=0){
		...
		array('fieldName'=>'显示状态','fieldValue'=>'status','fieldType'=>'radio','isMust'=>1,'fieldData'=>array(1=>'启用',2=>'禁用'),'attrExtend'=>'placeholder=""'),
