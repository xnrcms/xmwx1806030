...
class BusinessOrderController extends AdminController {
    ...
    public function index() {
        ...
        $map 				= array();
        //查询该用户的店铺id
        $shop_id 			= M('shop')->where(array('uid'=>BUID))->getField('id');
        $map['shop_id']		= $shop_id;
    ...
    public function view() {
        $id = I('get.id');
        $info = M('BusinessOrder')->field(true)->find($id);
        $status = array(
            0 => '',
            1 => '待付款',
            2 => '待发货',
    ...
    public function del() {
        ...
        if (M('BusinessOrder')->where($map)->delete()) {
            //记录行为
    ...
    */ 
    ...
    public function exchange(){
    	...
    	$result = M('BusinessOrder')->where(array('id'=>$id))->save(array('member_id'=>UID,'exchange_time'=>NOW_TIME));
    	if (false !== $result) {
    		//数据返回
