...
class ArticleController extends AdminController {
    ...
    public function index() {
        ...
        if (!empty($list)) {
            foreach ($list as $k => $v) {
                //数据格式化
            	$list[$k]['type'] = M('article_type')->where(array('id'=>$v['type']))->getField('type_name');
        ...
        if (empty($ParentCatName)) {
            $this->assign('SmallNav', array('系统消息管理', '系统消息列表'));
        } else {
            $cname[] = '系统消息管理';
    ...
    protected function CustomerForm($index = 0) {
    	...
    	foreach ($article_type as $key=>$value){
    		$type[$value['id']] = $value['type_name'];
    	}

        $FormData[0] = array(
        	array('fieldName' => '分类','fieldValue'=>'type','fieldType'=>'select','isMust'=>1,'fieldData'=>$type,'attrExtend'=>'placeholder=""'),
            array('fieldName' => '标题', 'fieldValue' => 'title', 'fieldType' => 'text', 'isMust' => 0, 'fieldData' => array(), 'attrExtend' => 'placeholder="请输入标题"'),
        	array('fieldName' => '图片', 'fieldValue' => 'pic', 'fieldType' => 'image', 'isMust' => 0, 'fieldData' => array(), 'attrExtend' => 'data-table="goods" data-field="goodsimg" data-size=""'),
        	array('fieldName' => '内容', 'fieldValue' => 'content', 'fieldType' => 'editor', 'isMust' => 0, 'fieldData' => array(), 'attrExtend' => 'placeholder="" rows="5" style="height:100%;"'),
            array('fieldName' => '显示状态', 'fieldValue' => 'status', 'fieldType' => 'radio', 'isMust' => 0, 'fieldData' => array(1 => '启用',0 => '禁用'), 'attrExtend' => 'placeholder=""'),
