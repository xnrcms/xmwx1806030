...
class GoodsAttributeController extends AdminController {
    ...
    public function edit($id = 0) {
        ...
        //页面数据
        $info = M('GoodsAttribute')->field(true)->find($id);
        if (false === $info) {
            $this->error('获取配置信息错误');
        }
        $this->assign('info', $info);
    ...
    public function del() {
        ...
        if (M('GoodsAttribute')->where($map)->delete()) {
            //记录行为
    ...
    protected function CustomerForm($index = 0) {
        $FormData[0] = array(
            array('fieldName'=>'规格名称','fieldValue'=>'avalue','fieldType'=>'text','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入属性值"'),
            array('fieldName'=>'属性价格','fieldValue'=>'price','fieldType'=>'text','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入属性价格"'),
            //array('fieldName'=>'属性价格(进价)','fieldValue'=>'sprice','fieldType'=>'text','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入属性价格(进价)"'),
            array('fieldName'=>'属性库存','fieldValue'=>'stock','fieldType'=>'text','isMust'=>0,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入属性库存"'),
