 ...
 * 后台配置控制器
 */
class FareController extends AdminController {
	/**
	 * 修改成自己的
	 * @author xxx
	 */
	...
	public function edit($id = 0){
		...
		//页面数据
		$info 			= M('fare')->field(true)->find($id);
		if(false === $info){
			$this->error('获取配置信息错误');
		}
		$this->assign('info', $info);
	...
	protected function CustomerForm($index=0){
		...
		$FormData[0] = array(
		array('fieldName'=>'省名','fieldValue'=>'area','fieldType'=>'text','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入省名"'),
		array('fieldName'=>'运费(金额)','fieldValue'=>'fee','fieldType'=>'text','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入运费(金额)"'),
