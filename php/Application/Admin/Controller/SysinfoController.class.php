 ...
 * 后台配置控制器
 */
class SysinfoController extends AdminController {
    ...
    public function edit($id = 0) {
        ...
        if (IS_POST)
            $this->update();
        //页面数据
        $info = M('Sysinfo')->field(true)->find($id);
        if (false === $info) {
            $this->error('获取配置信息错误');
        }
    ...
    protected function CustomerForm($index = 0) {
        ...
        $FormData[0] = array(
            array('fieldName' => '消息标题', 'fieldValue' => 'title', 'fieldType' => 'text', 'isMust' => 1, 'fieldData' => array(), 'attrExtend' => 'placeholder="请输入消息标题"'),
            array('fieldName' => '消息内容', 'fieldValue' => 'content', 'fieldType' => 'editor', 'isMust' => 0, 'fieldData' => array(), 'attrExtend' => 'placeholder="" rows="5" style="height:100%;"'),
            array('fieldName' => '显示状态', 'fieldValue' => 'status', 'fieldType' => 'radio', 'isMust' => 0, 'fieldData' => array(1 => '启用',0 => '禁用'), 'attrExtend' => 'placeholder=""'),
            array('fieldName' => '消息图标', 'fieldValue' => 'pic', 'fieldType' => 'image', 'isMust' => 1, 'fieldData' => array(), 'attrExtend' => 'data-table="goods" data-field="goodsimg" data-size=""'),
