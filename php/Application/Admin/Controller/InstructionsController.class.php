...
class InstructionsController extends AdminController {
	...
	public function edit($id = 0){
		...
		//页面数据
		$info 			= M('Instructions')->field(true)->find($id);
		if(false === $info){
			$this->error('获取配置信息错误');
		}
		$this->assign('info', $info);
	...
	public function del(){
		...
		if(M('Instructions')->where($map)->delete()){
			//记录行为
	...
	protected function CustomerForm($index=0){
		...
		$FormData[0] = array(
		array('fieldName'=>'内容','fieldValue'=>'content','fieldType'=>'editor','isMust'=>1,'fieldData'=>array(),'attrExtend'=>'placeholder="请输入内容"'),
		array('fieldName'=>'用户须知分类','fieldValue'=>'type','fieldType'=>'radio','isMust'=>1,'fieldData'=>array(1=>'免费试用攻略',2=>'分销攻略'),'attrExtend'=>'placeholder=""'),
