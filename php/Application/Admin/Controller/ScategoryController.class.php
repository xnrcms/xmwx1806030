...
class ScategoryController extends AdminController {
	...
	public function index(){
		...
		$list 						= D('Scategory')->getTree(0,'id,name,sort,pid,status',$map);
		if (!empty($list)){
	...
	public function gettree(){
		$tree 		= D('Scategory')->getTree(0,'id,name,sort,pid,status');
		$this->assign('tree', $tree);
		C('_SYS_GET_CATEGORY_TREE_', true); //标记系统获取分类树模板
	...
	public function edit($id = 0){
		if (IS_POST) $this->update();
		/* 获取数据 */
		$info 			= M('Scategory')->field(true)->find($id);
		if(false === $info){
	...
	public function del(){
		...
		foreach ($ids as $v){
			$child = M('Scategory')->where(array('pid'=>$v))->field('id')->select();
			if (!empty($child)){
				foreach ($child as $vv){
					if (!in_array($vv['id'], $ids)){
						$catname	= M('Scategory')->where(array('id'=>$v))->getField('name');
		...
		if(M('Scategory')->where($map)->delete()){
			//记录行为
			action_log('Scategory/del',$ids,UID);
			//数据返回
	...
	protected function update(){
		if(IS_POST){
			...
			if(false !== $res){
				//记录行为
				action_log('Scategory',$data['id'],UID);
				//数据返回
	...
	protected function CustomerForm($index=0){
		$menus 				= M('Scategory')->field(array('id','name as title','pid'))->where(array('pid'=>0))->select();
