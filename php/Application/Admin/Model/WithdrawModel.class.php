...
class WithdrawModel extends Model{
	...
	protected $_validate = array(
		array('status', 'require', '请选择审核状态', self::EXISTS_VALIDATE, 'regex', self::MODEL_UPDATE),
	);
	/*
	 * ===========自动完成定义说明==============
	 * Field		必填-需要操作的字段
	...
	public function update($updata = array()){
		...
		//提现表信息
		$info 			= $this->field('id,uid,xinlidou,xianglidou,money,status')->find($data['id']);
		if($info['status'] != 0){
			$this->error = '已经审核！';
			return false;
		}
		//修改状态
		$res = $this->save($data);
		if(false === $res) {$this->error = '审核失败！';return false;}
		//审核不通过退回金额
		if($data['status'] == 2){
			$row 								= array();
			$row['current_xinlidou'] 			= array('exp',"current_xinlidou+{$info['xinlidou']}");
			$row['current_xianglidou'] 			= array('exp',"current_xianglidou+{$info['xianglidou']}");
			$result = M('user')->where(array('id'=>$info['uid']))->save($row);
			if(false === $result) {
				$this->error = '鑫豆回退失败！';
				return false;
			}else{
				//交易日志
				/* M('transaction_log')->add(array(
						'uid'			=> $info['uid'],
						'description'	=> '提现审核未通过',
						'type'			=> 1,
						'symbol'		=> '+',
