...
class MenuModel extends Model
{
	...
	array('title','require','请输入菜单名称',self::MUST_VALIDATE),
	array('url', 'require', '请输入菜单URL', self::MUST_VALIDATE),
	...
	public function getSonMenu($mid,$map = array())
	{
		$base 		= array('pid'=>$mid,'hide'=>0);
		$where 		= array_merge($base,$map);
		$MenuList 	= $this->field('id,title,group,url,posttype')->where($where)->order("sort asc")->select();
		$arr 		= array();
		if (!empty($MenuList))
		{
	...
	{
		return intval(I('post.sort')) >0 ? intval(I('post.sort')) : 1;
	}
}