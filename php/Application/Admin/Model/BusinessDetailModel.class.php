 ...
 * apilist模型
 */
class BusinessDetailModel extends Model{
	/*
	 * ===========自动验证定义说明==============
	...
	protected $_validate = array(
		array('name', 'require', '请输入商家名称', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
	);
	/*
	 * ===========自动完成定义说明==============
	 * Field		必填-需要操作的字段
	...
	public function update($updata = array()){
		...
		$sinfo=M('Scategory')->where(array('id'=>$data['category_id']))->find();
		$num=M('Scategory')->where(array('id'=>$sinfo['pid']))->count();
		if($num<1){
			$this->error = '请选择二级分类！';return false;
		}
		/* 添加或更新数据 */
		if(empty($data['id'])){
			$res = $this->add();
