...
class CouponConfigModel extends Model{
	...
	protected $_validate = array(
		array('shakeNum', 'require', '请输入摇券次数', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
    	array('shakeNum', 'number', '摇券次数必须为正整数', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
    	array('probability', 'require', '请输入概率', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
    	array('probability', 'double', '概率必须为数字', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
	);
	/*
	 * ===========自动完成定义说明==============
	 * Field		必填-需要操作的字段
