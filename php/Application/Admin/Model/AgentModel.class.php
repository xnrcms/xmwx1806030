...
class AgentModel extends Model{
	...
	public function update($updata = array()){
		...
		//处理业务逻辑
		if($updata['goodstype']==1 && empty($updata['points'])){
				$this->error = '积分专区,请输入积分';return false;
		}elseif($updata['goodstype']==2 && empty($updata['goodsprice'])){
				$this->error = '现金专区,请输入价格';return false;
		}elseif($updata['goodstype']==3 && (empty($updata['antcurrency']) || empty($updata['goodsprice']))){
				$this->error = '折扣专区,请输入聚蚁币';return false;
		}
