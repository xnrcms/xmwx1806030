 ...
 * 企业支付接口
 */
class Copay_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["partner_trade_no"] == null) {
				throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数partner_trade_no！"."<br>");
			}elseif($this->parameters["openid"] == null){
				throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数openid！"."<br>");
			}elseif($this->parameters["check_name"] == null){
				throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数check_name！"."<br>");
			}elseif($this->parameters["amount"] == null){
				throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数amount！"."<br>");
			}elseif($this->parameters["desc"] == null){
				throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数desc！"."<br>");
			}elseif($this->parameters["spbill_create_ip"] == null){
				throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数spbill_create_ip！"."<br>");
			}
			$this->parameters["mch_appid"] = C('APPID');//公众账号ID
			$this->parameters["mchid"] = C('MCHID');//商户号
			$this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
			$this->parameters["sign"] = $this->getSign($this->parameters);//签名
			return  $this->arrayToXml($this->parameters);
		}catch (SDKRuntimeException $e)
